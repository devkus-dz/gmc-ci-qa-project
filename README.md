...
This project is a simple ES6 JavaScript Calculator. It was built to learn and practice professional software development skills: **Version Control (Git)**, **Continuous Integration (CI)**, and **Quality Assurance (QA)**.

## Steps We Completed

### 1. Quality Assurance (QA)

To make sure our code is high quality, we used two main tools:

- **Jest (Unit Testing):** We wrote a `Calculator.test.js` file to test our math logic. This proves our `add`, `subtract`, and `multiply` functions give the correct answers.
- **ESLint (Linter):** We configured ESLint to read our code and find messy style issues or missing variables. This keeps the code clean and easy to read.

### 2. Version Control (Git & GitHub)

We used Git to save our work and collaborate safely.

- We created a GitHub repository and linked it to our local project.
- We used **Branches** to work on new ideas safely. For example, we created a `feature-multiply` branch to add multiplication without breaking the `main` code.
- We opened a **Pull Request (PR)** on GitHub to review the new code before merging it together.

### 3. Continuous Integration (CI Pipeline)

We set up an automated robot using **GitHub Actions**.

- We created a `.github/workflows/ci.yml` file.
- Now, every time we push code to GitHub, the CI pipeline automatically installs Node.js, runs our ESLint linter, and runs our Jest unit tests.
- If the code is good, we get a green checkmark. If there is a bug, the pipeline fails and stops us from merging bad code.
